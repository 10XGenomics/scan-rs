...
#[derive(Default)]
pub(crate) struct FastLocalMoving {
    resolution: f64,
...
impl FastLocalMoving {
    ...
    pub fn iterate(&mut self, n: &Network, c: &mut impl Clustering, rng: &mut impl Rng) -> bool {
        ...
        self.stable_nodes.zero_len(n.nodes());
        ...
        loop {
            ...
            if best_cluster != current_cluster {
                ...
                for (neighbor, _) in n.neighbors(j) {
                    if self.stable_nodes[neighbor] && c.get(neighbor) != best_cluster {
                        self.stable_nodes[neighbor] = false;
                        num_unstable_nodes += 1;
                        self.node_order[(i + num_unstable_nodes) % n.nodes()] = neighbor;
                    }
                }
