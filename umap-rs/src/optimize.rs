...
//! This algorithm appear to give noticeably worse embeddings reasonably often.
//! This was first flagged by Paul Lund on the Flex team. It may converge more slowly
//! and therefore require more iterations, but this has not yet been tested.
...
fn optimize_layout_step(state: &mut State, thread_pool: Option<&rayon::ThreadPool>) {
    match state.distance_type.0 {
        ...
        _ => {
            // FIXME(pjm) - non-Euclidean distance version has not been parallelized yet
            for i in 0..state.num_samples() {
                iterate(state, i);
            }
        }
